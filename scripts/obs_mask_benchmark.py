...
def bind_masked_forward(model: ObsPerceiverLatent) -> None:
    ...
    def forward(self, td: TensorDict) -> TensorDict:  # type: ignore[override]
        ...
        assert self._use_mask
        assert "obs_mask" in td
        mask_value = -torch.finfo(k.dtype).max
        attn_bias = rearrange(td["obs_mask"].to(torch.bool), "b m -> b 1 1 m").to(k.dtype) * mask_value
...
def main():
    ...
    bind_masked_forward(model_mask)
