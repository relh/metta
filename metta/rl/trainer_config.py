...
class OptimizerConfig(Config):
    ...
    # Learning rate tuned from relh.cg.0204 sweep winners (schedule-free AdamW)
    learning_rate: float = Field(default=0.00924, gt=0, le=1.0)
    ...
    # Epsilon tuned from relh.cg.0204 sweep winners
    eps: float = Field(default=2.5e-06, gt=0)
    ...
    momentum: float = Field(default=0.9724, ge=0, le=1.0)  # Beta parameter for ScheduleFree
    warmup_steps: int = Field(default=1752, ge=0)  # Number of warmup steps for ScheduleFree
...
class SamplingConfig(Config):
    ...
    prio_alpha: float = Field(default=0.3098, ge=0, le=1.0)
    prio_beta0: float = Field(default=0.7994, ge=0, le=1.0)
...
class AdvantageConfig(Config):
    ...
    gamma: float = Field(default=0.9995, ge=0, le=1.0)
    gae_lambda: float = Field(default=0.9161, ge=0, le=1.0)
