...
SUBMISSION_REF_PATH_TEMPLATE = "devops/stable/state/{job_name}/cogames_submission_ref.json"
SUBMISSION_POLICY_NAME = "stable-release-check"
SUBMISSION_POLL_TIMEOUT_S = 1200
SUBMISSION_POLL_INTERVAL_S = 20


def _submission_ref_path_for_job(job_name: str) -> Path:
    return Path(SUBMISSION_REF_PATH_TEMPLATE.format(job_name=job_name))
...
@stable_function_check(
    ...
    output_references={"submission_ref_path": SUBMISSION_REF_PATH_TEMPLATE},
)
def upload_and_submit_policy(ctx: StableCheckContext) -> None:
    _ensure_cogames_auth_token()
    policy_name = SUBMISSION_POLICY_NAME
    with isolated_venv(packages=["cogames"]) as bin_dir:
        ...
        if result.returncode != 0:
            raise AssertionError(
                f"cogames upload failed:\nstdout:\n{result.stdout.strip()}\n\nstderr:\n{result.stderr.strip()}"
            )

    submission_ref_path = _submission_ref_path_for_job(ctx.job_name)
    submission_ref_path.parent.mkdir(parents=True, exist_ok=True)
    submission_ref_path.write_text(
        json.dumps(
            {
                "policy_name": policy_name,
                "season": SUBMISSION_SEASON,
                "server_url": PROD_STATS_SERVER_URI,
                "login_server": OBSERVATORY_AUTH_SERVER_URL,
            },
            indent=2,
        )
    )
...
@stable_function_check(
    ...
    depends_on=upload_and_submit_policy,
    input_references={"submission_ref_path": "submission_ref_path"},
)
def check_submission_results(ctx: StableCheckContext) -> None:
    _ensure_cogames_auth_token()

    submission_ref_path = ctx.inputs["submission_ref_path"]
    ref_payload = json.loads(Path(submission_ref_path).read_text())
    ...
    with isolated_venv(packages=["cogames"]) as bin_dir:
        while time.monotonic() < deadline:
            ...
            if failed > 0 and pending == 0:
                raise AssertionError(
                    f"Submission `{policy_name}` failed before completion "
                    f"(completed={completed}, failed={failed}, pending={pending})."
                )

            last_status = (
                f"waiting for completion: completed={completed}, failed={failed}, pending={pending}, "
                f"entries={len(entries) if isinstance(entries, list) else 'n/a'}"
            )
            time.sleep(SUBMISSION_POLL_INTERVAL_S)

    raise AssertionError(
        f"Timed out waiting for submission `{policy_name}` to complete in season `{season}`: {last_status}"
    )
