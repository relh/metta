...
**Usage:** `/cf:really /gt:fix-ci`, `/cf:really /gt:submit`, `/cf:really /gt:fix-branch`
...
| ----------------- | ------------------------------------------------------ | ---------------------------------------- |
| `gt:fix-ci`       | All CI check-runs pass (via commit SHA)                | Wait for CI completion (poll check-runs) |
| `gt:fix-comments` | All PR review comments addressed                       | None                                     |
| `gt:fix-branch`   | Branch synced, restacked, comments addressed, CI green | Wait for CI completion                   |
| `gt:submit`       | Branch submitted and CI passes                         | Wait for CI completion                   |
| `gt:fix-stack`    | All branches in stack have CI green                    | Wait for CI on each branch               |
| `gt:cool`         | No compat cruft remains, tests pass                    | None                                     |
...
If the skill doesn't trigger async processes (e.g., `gt:cool`, `gt:fix-comments`), skip directly to Step 4.
...
- **gt:fix-ci-really** - Use `/cf:really /gt:fix-ci` instead
