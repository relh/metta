...
## Difference from /st.extract

| Aspect          | /st.extract                      | /st.extract-copy                   |
| --------------- | -------------------------------- | ---------------------------------- |
...
| Submit          | `/pr.submit`                    | Push new branch only       |
...
- **/pr.submit** - For submitting the new branch

**Pairs with:**

- **/st.extract** - When you want to remove from original (destructive)
- **/st.make-stack** - For sequential splitting
- **/pr.fix-branch** - Fix issues in the copied branch
