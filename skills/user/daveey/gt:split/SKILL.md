...
Group changes into the requested branches. Follow the same principles as `/gt:make-stack`:
...
**Run /gt:submit for each branch** to test, clean, and submit:
...
- **gt:submit** - Called for each new branch after creation
...
- **gt:make-stack** - Similar but auto-analyzes groupings; gt:split is more user-directed
- **gt:fix-stack** - Fix issues across the new stack later
- **gt:extract** - For parallel branches instead of sequential
