...
- Stacking independent features (use `/gt:extract` for parallel branches instead)
...
- Run `/gt:cool` to clean up any backwards compat code
...
| Submit          | `/gt:submit`                    | Test, clean, submit          |
| Restack upstack | `gt track --parent`             | Reconnect upstack branches   |

## When to Use /gt:extract or /gt:extract-copy Instead

**Use /gt:make-stack when:** Changes are sequential/dependent (branch-2 needs branch-1)

```
parent → branch-1 → branch-2 → branch-3
```

**Use /gt:extract when:** Changes are independent/parallel AND you want to split permanently
...
**Use /gt:extract-copy when:** You want to land a subset early WITHOUT modifying the original
...
| ----------------------------- | ---------------- | ----------------------- |
| Split into sequential stack   | /gt:make-stack   | Yes (replaced by stack) |
| Split into parallel branches  | /gt:extract      | Yes (feature removed)   |
| Copy subset for early landing | /gt:extract-copy | No (unchanged)          |

**If your branch has both sequential and parallel features:**

1. First use `/gt:extract-copy` to copy independent features for early landing
2. Then use `/gt:make-stack` on the original to create the sequential stack
...
/gt:extract-copy  # Copy auth into parallel branch
...
/gt:make-stack  # Split into api → dashboard → auth
...
2. **Extract shared code first** - Use `/gt:extract` to create a "foundation" branch with shared utilities
...
- **gt:submit** - Called for each branch after creation (tests, /gt:cool, submit)
- **gt:extract** - Use first to separate independent features into parallel branches
- **gt:extract-copy** - Copy subset for early landing without modifying original

**Pairs with:**

- **gt:fix-branch** - Fix issues in individual stack branches later
- **gt:fix-stack** - Fix the entire stack after creation
- **gt:extract** - For independent features that should be parallel, not stacked
- **gt:extract-copy** - For landing a subset early while continuing full development
