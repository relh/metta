...
By default, work in an isolated git worktree. **Skip if already in worktree** (e.g., called from /gt:submit).
...
- **gt:submit** - As Step 4, after tests pass and before final submission (worktree already set up)

**Pairs with:**

- **test-driven-development** - Ensure tests catch missing callsites
- **gt:fix-ci** - May be needed if /gt:cool introduces test failures
