...
digraph learn_skills {
  ...
  make [label="/sk.make-skill (external)"];
...
- A `SKILL.md` inside a named directory (e.g., `pr.fix-ci/SKILL.md`), OR
...
|---|------|-------------|------|
| 1 | pr.fix-ci | Fix CI failures... | directory |
...
### Case B: Outside a git repo or different machine (copy via /sk.make-skill)

The skill can't be symlinked reliably. Instead, invoke `/sk.make-skill` to recreate it:

1. Read the full content of the source skill
2. Invoke `/sk.make-skill <name> "<description>"`
3. Pass the source skill's content as the basis for the new skill
4. Let `sk.make-skill` handle placement, symlinking, and README updates
...
|------|--------|----------|
| pr.fix-ci | symlink | ~/.claude/skills/pr.fix-ci → /path/to/source |
...
| External | Use /sk.make-skill to copy the skill over              |
| Report   | Show what was imported and how to use it               |

## Integration

**Uses:** `sk.make-skill` (for copying external skills) **Called by:** User directly when discovering new skills **Pairs
