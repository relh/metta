...
_METRIC_ALIASES: dict[str, tuple[str, ...]] = {
    "action.move.success": ("action.move",),
...
def _metric_aliases(key: str) -> tuple[str, ...]:
    return (key, *_METRIC_ALIASES.get(key, ()))
...
def metric_present(metrics: Mapping[str, float], key: str) -> bool:
    return any(alias in metrics for alias in _metric_aliases(key))
...
def action_success_total(metrics: Mapping[str, float]) -> float:
    ...
    if "action.move.success" not in metrics and isinstance(move_alias, (int, float)):
