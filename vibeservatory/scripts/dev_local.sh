...
Runs Vibeservatory dashboard + bardo frontends against production read-only DB via k8s tunnel.
Also starts pantheon + diagnose frontends for shared-route smoke checks.
EOF
  exit 0
fi
...
export BARDO_PROXY_URL="${BARDO_PROXY_URL:-http://127.0.0.1:5175/bardo}"
export PANTHEON_PROXY_URL="${PANTHEON_PROXY_URL:-http://127.0.0.1:5176/pantheon}"
export DIAGNOSE_PROXY_URL="${DIAGNOSE_PROXY_URL:-http://127.0.0.1:5177/diagnose}"
...
echo "[vibeservatory] Bardo:    http://127.0.0.1:5175/bardo"
echo "[vibeservatory] Pantheon: http://127.0.0.1:5176/pantheon"
echo "[vibeservatory] Diagnose: http://127.0.0.1:5177/diagnose"
...
cleanup() {
  ...
  if [[ -n "${BARDO_FRONTEND_PID:-}" ]]; then
    kill "$BARDO_FRONTEND_PID" > /dev/null 2>&1 || true
  fi
  if [[ -n "${PANTHEON_FRONTEND_PID:-}" ]]; then
    kill "$PANTHEON_FRONTEND_PID" > /dev/null 2>&1 || true
  fi
  if [[ -n "${DIAGNOSE_FRONTEND_PID:-}" ]]; then
    kill "$DIAGNOSE_FRONTEND_PID" > /dev/null 2>&1 || true
  fi
...
pnpm --dir pantheon dev &
PANTHEON_FRONTEND_PID=$!

pnpm --dir diagnose dev &
DIAGNOSE_FRONTEND_PID=$!

wait -n "$BACKEND_PID" "$FRONTEND_PID" "$BARDO_FRONTEND_PID" "$PANTHEON_FRONTEND_PID" "$DIAGNOSE_FRONTEND_PID"
STATUS=$?
exit "$STATUS"
