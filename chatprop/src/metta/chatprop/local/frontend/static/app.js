...
function readBasePath() {
  const meta = document.querySelector('meta[name="chatprop-base-path"]')
  if (!meta) return ''
...
async function ensureMermaidClient() {
  if (!mermaidClientPromise) {
    mermaidClientPromise = import('https://cdn.jsdelivr.net/npm/mermaid@11/dist/mermaid.esm.min.mjs').then((module) => {
...
async function saveFlowchartArtifacts() {
  ...
  try {
    ...
    setFlowchartStatus(`Saved flowchart: ${payload.mermaid_path}${payload.json_path ? ` | ${payload.json_path}` : ''}`)
  } catch (error) {
...
function renderFindResponse(payload) {
  const lines = [`Branches: ${(payload.branches || []).join(', ')}`, `Matches: ${payload.count || 0}`, '']
...
function filterSessionRows(rows, filterText) {
  ...
  const needle = filterText.toLowerCase()
  return rows.filter((row) => {
    const target = [row.session_id, row.source, ...(row.branches || [])].filter(Boolean).join(' ').toLowerCase()
    return target.includes(needle)
  })
}
