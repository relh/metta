...
def _find_repo_root() -> Path:
    ...
    for candidate in [this_file.parent, *this_file.parents]:
        if (candidate / "packages" / "cogames-rl-researcher" / "prompts").exists():
            return candidate
    raise RuntimeError("Could not find repo root containing packages/cogames-rl-researcher/prompts")


def prompt_path_for_profile(profile: WorkflowProfile, repo_root: Path | None = None) -> Path:
    root = repo_root or _find_repo_root()
    path = root / "packages" / "cogames-rl-researcher" / "prompts" / PROMPT_BY_PROFILE[profile]
    if not path.exists():
...
set +e
uv run ./packages/cogames-rl-researcher/scripts/run_ai_researcher_startup.py \\
  --policy "$POLICY_SPEC" \\
