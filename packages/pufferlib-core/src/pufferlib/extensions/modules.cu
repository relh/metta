...
static void check_precision_dtype(const torch::Tensor& t, const char* name) {
#ifdef PRECISION_FLOAT
    TORCH_CHECK(
        t.scalar_type() == torch::kFloat32,
        name,
        " must be float32 (this build uses PRECISION_FLOAT)"
    );
#else
    TORCH_CHECK(
        t.scalar_type() == torch::kBFloat16,
        name,
        " must be bfloat16 (this build defaults to bf16; compile with -DPRECISION_FLOAT for float32)"
    );
#endif
}
...
std::vector<torch::Tensor> mingru_gate(
    ...
    check_precision_dtype(state, "state");
...
public:
    static torch::autograd::tensor_list forward(
        ...
        check_precision_dtype(combined, "combined");
...
public:
    static torch::autograd::tensor_list forward(
        ...
        check_precision_dtype(logits, "logits");
        check_precision_dtype(values_pred, "values_pred");
        check_precision_dtype(old_logprobs, "old_logprobs");
        check_precision_dtype(prio, "prio");
        check_precision_dtype(values, "values");
        check_precision_dtype(returns, "returns");
        check_precision_dtype(ratio_out, "ratio_out");
        check_precision_dtype(newvalue_out, "newvalue_out");

        bool is_continuous = logstd.defined() && logstd.numel() > 0;
        if (is_continuous) {
            check_precision_dtype(logstd, "logstd");
        }
...
void sample_logits(
    ...
    check_precision_dtype(logits, "logits");
    check_precision_dtype(value, "value");
    check_precision_dtype(logprobs_out, "logprobs_out");
    check_precision_dtype(value_out, "value_out");
