...
static std::tuple<torch::Tensor, torch::Tensor> mingru_gate_tuple(torch::Tensor state, torch::Tensor combined) {
  auto out = mingru_gate(std::move(state), std::move(combined));
  TORCH_CHECK(out.size() == 2, "mingru_gate must return exactly 2 tensors, got ", out.size());
  return {std::move(out[0]), std::move(out[1])};
}

static std::tuple<torch::Tensor, torch::Tensor> fused_scan_checkpointed_tuple(torch::Tensor combined, torch::Tensor state) {
  auto out = fused_scan_checkpointed(std::move(combined), std::move(state));
  TORCH_CHECK(out.size() == 2, "fused_scan_checkpointed must return exactly 2 tensors, got ", out.size());
  return {std::move(out[0]), std::move(out[1])};
}
...
static torch::Tensor fused_ppo_loss_optimized_tensor(
  ...
  auto out = fused_ppo_loss_optimized(
      std::move(logits),
      std::move(logstd),
      std::move(values_pred),
      std::move(actions),
      std::move(old_logprobs),
      std::move(advantages),
      std::move(prio),
      std::move(values),
      std::move(returns),
      ...
      std::move(adv_var),
      std::move(ratio_out),
      std::move(newvalue_out),
      std::move(act_sizes),
      static_cast<float>(clip_coef),
      static_cast<float>(vf_clip_coef),
      static_cast<float>(vf_coef),
      static_cast<float>(ent_coef));
  TORCH_CHECK(out.size() == 1, "fused_ppo_loss_optimized must return exactly 1 tensor, got ", out.size());
  return std::move(out[0]);
}

static void sample_logits_void(
    torch::Tensor logits,
    torch::Tensor logstd,
    torch::Tensor value,
  ...
  sample_logits(
      ...
      std::move(logstd),
      std::move(value),
      std::move(actions_out),
      std::move(logprobs_out),
      std::move(value_out),
      std::move(act_sizes),
