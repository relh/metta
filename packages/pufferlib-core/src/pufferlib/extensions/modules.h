...
std::vector<torch::Tensor> mingru_gate(torch::Tensor state,
                                       torch::Tensor combined);

// Fused scan with checkpointing (training path)
torch::autograd::tensor_list fused_scan_checkpointed(torch::Tensor combined,
                                                     torch::Tensor state);
...
torch::autograd::tensor_list fused_ppo_loss_optimized(
    ...
    torch::Tensor newvalue_out, torch::Tensor act_sizes, float clip_coef,
    float vf_clip_coef, float vf_coef, float ent_coef);

// Fused sample_logits: handles both discrete and continuous action sampling
void sample_logits(torch::Tensor logits, torch::Tensor logstd,
                   torch::Tensor value, torch::Tensor actions_out,
                   torch::Tensor logprobs_out, torch::Tensor value_out,
                   torch::Tensor act_sizes, uint64_t seed,
                   torch::Tensor offset);
