...
uv run packages/cogames-agents/scripts/run_cogsguard_instrumented_audit.py \
  --steps 200 \
  --agents 4 \
...
```bash
uv run pytest -q \
  packages/cogames-agents/tests/test_cogsguard_determinism.py \
  packages/cogames-agents/tests/test_cogsguard_guardrails.py \
  packages/cogames-agents/tests/test_rollout_trace.py
```
