...
const
  ConfigFileName = "vibescope_state.json"
...
proc getConfigPath(): string =
  ...
  result = getHomeDir() / ".config" / "vibescope"
  discard existsOrCreateDir(result)
  result = result / ConfigFileName

proc saveVibescopeState*(state: VibescopeState) =
  ## Save the full vibescope state to a config file.
  let configPath = getConfigPath()
  try:
    writeFile(configPath, $(%state))
  except:
    echo "Failed to save vibescope state: ", getCurrentExceptionMsg()

proc loadVibescopeState*(): VibescopeState =
  ## Load the vibescope state from a config file.
  let configPath = getConfigPath()
  result = VibescopeState(
  ...
  if fileExists(configPath):
    try:
      ...
      echo "Failed to load vibescope state: ", getCurrentExceptionMsg()
