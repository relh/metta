...
proc getAoeRange(typeName: string): int =
  ...
  # Get the config for this type - agents are under game.agent, objects under game.objects.
  var objConfig: JsonNode = nil
  if typeName == "agent":
    if "agent" in game:
      objConfig = game["agent"]
  else:
    if "objects" in game and typeName in game["objects"]:
      objConfig = game["objects"][typeName]
  if objConfig.isNil:
    return 0
  ...
  var maxRange = 0
  # Handle both dict (JObject) and array (JArray) formats for backwards compatibility.
  if aoes.kind == JObject:
    for name, aoe in aoes.pairs:
      # Check both "radius" (Python) and "range" (legacy) field names.
      var r = 0
      if "radius" in aoe:
        if aoe["radius"].kind == JInt:
          r = aoe["radius"].getInt
        elif aoe["radius"].kind == JFloat:
          r = aoe["radius"].getFloat.int
      elif "range" in aoe:
...
proc drawAOEOverlay*() =
  ...
  ## Colors determined by collective name: Cogs = green, Clips = red, Neutral = grey.
  if replay.isNil:
    return
...
proc drawPlannedPath*() =
  ...
  if playMode != Realtime or step != replay.maxSteps - 1:
    return
  # Copy keys to avoid iteration issues if table is modified elsewhere.
  ...
  for agentId in agentIds:
    ...
    let numActions = pathActions.len
    for i in 0 ..< numActions:
      let action = pathActions[i]
      if action.kind != Move:
        continue
    ...
    if agentObjectives.hasKey(agentId):
      ...
      let numObjectives = objectives.len
      if numObjectives > 0:
