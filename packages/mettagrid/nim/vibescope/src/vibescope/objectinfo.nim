...
proc getAoeConfigs(typeName: string): JsonNode =
  ## Get the AOE configs array for an object type or agent. Returns nil if no AOEs.
  if replay.isNil or replay.mgConfig.isNil:
    return nil
...
proc getAoeSourcesAffectingAgent(agent: Entity): seq[AoeSourceEffect] =
  ## Find all AOE sources (objects and agents) that affect this agent.
  result = @[]
  if replay.isNil:
    return
  ...
  for i in 0 ..< numObjects:
    ...
    for aoeConfig in aoeConfigs:
      ...
      if obj.id == agent.id:
        var effectSelf = false
        if "effect_self" in aoeConfig and aoeConfig["effect_self"].kind == JBool:
          effectSelf = aoeConfig["effect_self"].getBool
        if not effectSelf:
          continue
      # Get filter settings
...
proc drawObjectInfo*(panel: panels.Panel, frameId: string, contentPos: Vec2, contentSize: Vec2) =
