...
when defined(emscripten):
  ...
  EM_JS(void, setup_postmessage_replay_handler_internal, (void* userData), {
    ...
    window.parent.postMessage({ type: 'vibescopeReady' }, '*');
    ...
    window.addEventListener('message', function(event) {
      ...
      try {
        ...
        Module._vibescope_postmessage_replay_callback(userData, fileNamePtr, binaryPtr, binaryLength);
  ...
  proc vibescope_postmessage_replay_callback(userData: pointer, fileNamePtr: cstring, binaryPtr: pointer, binaryLen: cint) {.exportc, cdecl, codegenDecl: "EMSCRIPTEN_KEEPALIVE $# $#$#".} =
