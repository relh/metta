...
MettaScope uses a texture atlas system (`silky`) for efficient rendering. The atlas is generated at **runtime** when the
standalone binary starts (in the `when isMainModule` block).
