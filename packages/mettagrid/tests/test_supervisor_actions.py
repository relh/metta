...
def test_split_supervisor_actions_maps_compact_non_vibe_indices() -> None:
    teacher_actions = np.array([0, 2], dtype=np.int32)
    vibe_actions = np.zeros_like(teacher_actions)
    non_vibe_action_ids = np.array([4, 5, 6], dtype=np.int32)

    split_supervisor_actions_inplace(
        teacher_actions,
        vibe_actions,
        supervisor_non_vibe_action_ids=non_vibe_action_ids,
