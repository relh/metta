...
class VibescopeRenderer(Renderer):
    """Renderer for GUI mode using vibescope."""
    ...
    def __init__(self, autostart: bool = False):
        ...
        import vibescope  # type: ignore[import-not-found]  # noqa: PLC0415

        self._vibescope = vibescope
        self._data_dir = str(nim_root / "data") if nim_root else "."
        os.environ.setdefault("VIBESCOPE_DISABLE_CTRL_C", "1")
    ...
    def on_episode_start(self) -> None:
        ...
        # vibescope.init requires data_dir, replay, and autostart arguments
        json_str = json.dumps(initial_replay, allow_nan=False)
        try:
            self.response = self._vibescope.init(self._data_dir, json_str, self._autostart)
        except KeyboardInterrupt:
            logger.info("Interrupt received during vibescope init; ending episode.")
    ...
    def render(self) -> None:
        ...
        # Get collective inventories for vibescope commons panel
        ...
        try:
            self.response = self._vibescope.render(self._sim.current_step, json.dumps(step_replay, allow_nan=False))
        except KeyboardInterrupt:
            logger.info("Interrupt received during vibescope render; ending episode.")
            self._sim.end_episode()
            return
...
#
# Source: packages/mettagrid/nim/vibescope/bindings/generated
#   - The canonical location where `nim build` outputs bindings
#   - Present when running from a repo checkout
#
# Packaged: <site-packages>/mettagrid/nim/vibescope/bindings/generated
...
def _resolve_nim_root() -> Optional[Path]:
    # Source location (repo checkout): packages/mettagrid/nim/vibescope
    # This will not exist when installed in packaged form
    source = _python_package_root.parent.parent.parent / "nim" / "vibescope"

    # Packaged location (installed wheel): <site-packages>/mettagrid/nim/vibescope
    packaged = _python_package_root / "nim" / "vibescope"
...
#         try:
#             # Import the vibescope module
#             import vibescope

#             # Verify the module has the expected attributes
#             required_attrs = ["init", "render", "MettascopeError"]
#             missing_attrs = [attr for attr in required_attrs if not hasattr(vibescope, attr)]

#             if missing_attrs:
#                 # List what attributes are actually available
#                 available_attrs = [attr for attr in dir(vibescope) if not attr.startswith("_")]
#                 raise ImportError(
#                     f"vibescope module is missing required attributes: {missing_attrs}. "
...
#             def init(replay):
#                 return vibescope.init(data_dir=str(nim_root / "data"), replay=replay)

#             render = vibescope.render
#             VibescopeError = vibescope.VibescopeError

#         except ImportError as e:
#             raise ImportError(
#                 f"Failed to import vibescope from {nim_bindings_path}: {e}. "
...
#         raise ImportError(
#             "Could not find vibescope bindings. "
