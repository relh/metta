        ...
        try:
                ...
                try:
                    ...
                    if should_create_new:
                        ...
                        if still_need_create:
                            ...
                            logger.info(
                                f"Created new map for key {cache_key} (cached maps: {len(maps_list)}/{maps_per_key})"
                            )
                            return game_map
                        else:
            ...
            if should_create_new:
                ...
                if still_need_create:
                    ...
                    registry[cache_key] = maps_list
                    _save_registry(registry)
                    logger.info(f"Created new map for key {cache_key} (cached maps: {len(maps_list)}/{maps_per_key})")
                    return game_map
                else:
                    # Another process created it, use the cached one instead
                    random_idx = random.randint(0, len(maps_list) - 1)
