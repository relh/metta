...
class MockTeamsCommissioner(MockMatchExecutionMixin, TeamCommissionerBase):
    ...
    initial_config = TeamTournamentConfig(
        ...
        stages=[
            ...
            PolicyEvalStage(display_name="Play-ins: four-player", policies_per_team=4, matches_per_combo=1),
            SampleStage(display_name="Sampling: eight-player", team_size=8, num_teams=48, min_per_policy=4),
            TeamEvalStage(display_name="Playoffs: eight-player", matches_per_team=2, cull_fraction=0.5),
            TeamEvalStage(display_name="Playoffs: eight-player", matches_per_team=2, cull_fraction=0.5),
