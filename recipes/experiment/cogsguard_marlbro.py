...
def _with_role_conditional(variants: str | Sequence[str] | None) -> tuple[str, ...]:
    ...
    if "forced_role_vibes" not in variant_names:
        variant_names.append("forced_role_vibes")
...
def _apply_role_ids(env: MettaGridConfig, role_ids: Sequence[int]) -> None:
    ...
    for agent_cfg, role_id in zip(env.game.agents, role_ids, strict=False):
        ...
        agent_cfg.inventory.initial = {**agent_cfg.inventory.initial, "role_id": role_id}
...
def _set_role_id_assignment(curriculum: CurriculumConfig, role_ids: Sequence[int]) -> None:
    def _apply(task_generator_config: object) -> None:
        ...
        if hasattr(task_generator_config, "overrides"):
            overrides = dict(task_generator_config.overrides)
            # Force role-dependent vibes to stay stable for this setup.
            overrides["game.actions.change_vibe.enabled"] = False
            task_generator_config.overrides = overrides
