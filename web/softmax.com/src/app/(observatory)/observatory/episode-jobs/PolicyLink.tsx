...
import { PolicyVersionSummary } from "@observatory/lib/repo";
...
export const PolicyLink: FC<{
  uri: string;
  policy?: PolicyVersionSummary | null;
}> = ({ uri, policy }) => {
  if (policy) {
    const label =
      policy.name && policy.version !== null
        ? `${policy.name}:v${policy.version}`
        : policy.id.slice(0, 8);
    return (
      <StyledLink
        href={policyVersionRoute(policy.id)}
  ...
  if (parsed) {
    return (
      <StyledLink
        href={policyVersionRoute(parsed.uuid)}
        className="font-mono text-xs"
        title={uri}
      >
