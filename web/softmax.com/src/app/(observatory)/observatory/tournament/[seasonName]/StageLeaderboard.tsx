...
const LeaderboardTable = <Row,>({
  ...
  if (rows.length === 0) {
    return (
      <div className="text-foreground-muted py-4 text-center">{emptyText}</div>
    );
  }
      ...
      <div className="flex flex-wrap gap-1">
        {team.cogs.map((cog) => (
          <StyledLink
            ...
            className="bg-surface-alt border-border inline-block rounded border px-2 py-0.5 font-mono text-xs"
          >
  ...
  },
  {
    ...
    render: (team) => (
      <span
        className={
          team.eliminated
            ? "rounded bg-red-100 px-2 py-1 text-xs font-medium text-red-800 dark:bg-red-900/30 dark:text-red-400"
            : "rounded bg-green-100 px-2 py-1 text-xs font-medium text-green-800 dark:bg-green-900/30 dark:text-green-400"
        }
      >
...
function getScorePolicyColumns(
  ...
  return [
    ...
    },
    {
      ...
      render: (entry) => (
        <span className="font-mono text-sm">
          {entry.placement_score.toPrecision(4)}
        </span>
      ),
    },
    {
          ...
          <span className="font-mono text-sm">
            {(() => {
              const contributingRanks =
                topK === null
                  ? entry.team_ranks
                  : entry.team_ranks.slice(0, topK);
              const missingCount =
                topK === null
                  ? 0
                  : Math.max(0, topK - contributingRanks.length);
              const contributingSum = contributingRanks.reduce(
                (sum, rank) => sum + rank,
                0,
              );
              const penaltyPerMissing =
                missingCount > 0
                  ? (entry.placement_score - contributingSum) / missingCount
                  : null;
              const roundedPenalty =
                penaltyPerMissing === null
                  ? null
                  : Math.round(penaltyPerMissing);
              const penaltyLabel =
                penaltyPerMissing === null
                  ? null
                  : Math.abs(
                        penaltyPerMissing -
                          (roundedPenalty ?? penaltyPerMissing),
                      ) < 1e-6
                ...
                <>
                  ...
                  {penaltyLabel !== null &&
                    Array.from({ length: missingCount }, (_value, index) => (
                      ...
                      >
                        {entry.team_ranks.length > 0 || index > 0 ? ", " : ""}+
                        {penaltyLabel}
                      </span>
                    ))}
                </>
...
const StageLeaderboardContent: FC<{
  ...
  const leaderboard = await getStageLeaderboard(
    repo,
    seasonName,
    stage,
    stageKind,
