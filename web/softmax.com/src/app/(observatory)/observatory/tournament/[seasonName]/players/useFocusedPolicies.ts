...
import {
  FOCUS_POLICY_QUERY_KEY,
  parseFocusedPolicyIds,
  withFocusedPolicyIds,
} from "./focusPolicyQuery";
...
export function useFocusedPolicies({
  ...
  const validPolicyIdSet = useMemo(
    () => new Set(validPolicyIds),
    [validPolicyIds],
  );

  const focusedPolicyIds = useMemo(() => {
    const rawPolicyIds = parseFocusedPolicyIds(
      searchParams.get(FOCUS_POLICY_QUERY_KEY),
    );
    return rawPolicyIds.filter((policyId) => validPolicyIdSet.has(policyId));
  }, [searchParams, validPolicyIdSet]);

  const focusedPolicyIdSet = useMemo(
    () => new Set(focusedPolicyIds),
    [focusedPolicyIds],
  );
  ...
  const setFocusedPolicyIds = useCallback(
    (policyIds: string[]) => {
      const nextPolicyIds = policyIds.filter((policyId) =>
        validPolicyIdSet.has(policyId),
      );
      const next = withFocusedPolicyIds(
  ...
  const toggleFocusedPolicy = useCallback(
    (policyId: string) => {
      ...
      const orderedFocusedPolicyIds = orderedPolicyIds.filter((id) =>
        nextSet.has(id),
      );
