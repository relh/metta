...
import { matchesRoute, policyVersionRoute } from "@observatory/lib/routes";
  ...
  const sortedRows = useMemo(() => {
    ...
    nextRows.sort((a, b) => {
      if (sortState?.key === "player") {
        const byPlayer = a.policyLabel.localeCompare(b.policyLabel, undefined, {
          sensitivity: "base",
        });
        if (byPlayer !== 0)
        ...
        if (byEntered !== 0)
          return sortState.direction === "asc" ? byEntered : -byEntered;
      ...
      <TableHeader>
          ...
          <div className="inline-flex flex-col items-center gap-0.5">
            ...
            <div className="inline-flex items-center gap-1">
              <button
                ...
                className={clsx(
                  "cursor-pointer border-0 bg-transparent p-0 text-[10px]",
              ...
              <button
                ...
                className="text-foreground-muted hover:text-foreground disabled:text-foreground-muted cursor-pointer border-0 bg-transparent p-0 text-sm leading-none disabled:cursor-default"
        ...
        <TH className="text-left" aria-sort={getSortAria("player")}>
          <button
            ...
            className="font-inherit inline-flex cursor-pointer items-center gap-0.5 border-0 bg-transparent p-0 text-inherit"
          >
            <span>Player</span>
            <span className="text-foreground-muted text-[10px]">
              {getSortIndicator("player")}
            </span>
          </button>
        </TH>
        {showEnteredColumn ? (
            ...
            <TR key={row.policyId}>
              <TD className="text-center">
                <button
                  ...
                  className={clsx(
                    "inline-flex h-5 w-5 cursor-pointer items-center justify-center rounded border text-[11px] leading-none transition-colors",
                    isFocused
                      ? "text-white"
                      : "border-border-subtle hover:border-foreground-muted hover:text-foreground-muted text-transparent",
                  )}
                  style={
                    isFocused
                      ? {
                  ...
                  title={
                    isFocused
                      ? `Unfocus ${row.policyLabel}`
                      : `Focus ${row.policyLabel}`
                  }
                  aria-label={
                    isFocused
                      ? `Unfocus ${row.policyLabel}`
              ...
              <TD className="text-left">
                <StyledLink
                  href={policyVersionRoute(row.policyId)}
              ...
              {showEnteredColumn ? (
                <TD className="text-foreground-muted text-left text-sm">
                  {row.enteredAtLabel}
                </TD>
              ) : null}
              {columns.map((column) => {
                ...
                if (!stage || !stage.hasPool) {
                  return (
                    <TD
                      key={column.key}
                      className={clsx(
                        "text-foreground-muted text-right",
                        column.selected ? "bg-blue-950/10" : "",
                      )}
                    >
                      -
                    </TD>
                    ...
                    <div className="flex flex-col items-end gap-0.5 text-right">
                      ...
                      <Link
                        href={matchesRoute(seasonName, {
                          ...
                          pool_names: hasTournamentProgress
                            ? undefined
                            : [column.key],
                          policy_version_ids: [row.policyId],
                        })}
                        className="text-foreground-muted cursor-pointer text-xs no-underline transition-colors hover:text-blue-600 dark:hover:text-blue-400"
                      >
