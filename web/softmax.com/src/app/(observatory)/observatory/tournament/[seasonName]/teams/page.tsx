...
import type { TeamSummary } from "@observatory/lib/api";
...
export default async function TeamsPage({
  params,
  searchParams,
}: PageProps<"/observatory/tournament/[seasonName]/teams">) {
  ...
  const selectedTeamStage =
    selectedStage && teamStages.some((stage) => stage.name === selectedStage)
      ? selectedStage
      : null;
  ...
  if (
    selectedStage &&
    (selectedStageStatus === "pending" || !selectedTeamStage)
  ) {
    return (
      <Card title={cardTitle}>
        <div className="text-foreground-muted py-4 text-center">
          No team results for this stage yet (
          {selectedStageTitle ?? stageLabel(selectedStage)}).
        </div>
      </Card>
    );
  }
  ...
  for (const group of compositionMap.values()) {
    ...
    const rowCount = Math.max(
      ...teamStages.map((stage) => group.byStage.get(stage.name)?.length ?? 0),
    );
  ...
  rows.sort((a, b) => {
    if (selectedTeamStage) {
      const aScore =
        a.byStage.get(selectedTeamStage)?.score ?? Number.NEGATIVE_INFINITY;
      const bScore =
        b.byStage.get(selectedTeamStage)?.score ?? Number.NEGATIVE_INFINITY;
      if (aScore !== bScore) return bScore - aScore;
    }
    if (a.byStage.size !== b.byStage.size)
      return b.byStage.size - a.byStage.size;
                    ...
                    <div className="flex flex-wrap gap-1">
                      {row.cogs.map((cog) => (
                        <StyledLink
                          ...
                          className="bg-surface-alt border-border inline-block rounded border px-2 py-0.5 font-mono text-xs transition-colors hover:border-blue-400"
                        >
                      ...
                      >
                        <div className="flex flex-col items-start gap-1">
                          ...
                          <span
                            className={
                              team.eliminated
                                ? "rounded bg-red-100 px-2 py-0.5 text-xs font-medium text-red-800 dark:bg-red-900/30 dark:text-red-400"
                                : "rounded bg-green-100 px-2 py-0.5 text-xs font-medium text-green-800 dark:bg-green-900/30 dark:text-green-400"
                            }
                          >
