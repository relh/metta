...
export function PolicyDashboardEmbed({ src }: { src: string }) {
  ...
  useEffect(() => {
    ...
    const fetchSessionToken = async (): Promise<{
      ...
      const response = await fetch("/api/policy-dashboard/session-token", {
        method: "POST",
        cache: "no-store",
      });
      if (!response.ok) {
        throw new Error(
          `Failed to create policy dashboard token (${response.status})`,
        );
      }
    ...
    <>
      {bootstrapError ? (
        <div className="mb-2 rounded bg-red-500/15 px-3 py-2 text-sm text-red-500">
          Unable to prepare policy dashboard authentication: {bootstrapError}
        </div>
      ) : null}
      <iframe
