...
function stageKindToLeaderboardType(
...
export async function getSeasonStageContext(
  ...
  const selectedStageKind =
    progress && selectedStage
      ? stageKindForPool(progress.stage_flow, selectedStage)
      : null;
  const scorePoliciesStage =
    progress?.stage_flow.find((stage) => stage.kind === "score_policies") ??
    null;
...
export function getTeamStages(
  ...
  return progress.stage_flow
    ...
    .filter(
      (stage): stage is StageStats =>
        stage !== undefined && (stage.team_count ?? 0) > 0,
    );
}

export function getPlayerStages(
  stages: StageStats[],
  progress: ProgressResponse | null,
): StageStats[] {
  if (!progress) {
...
export async function getStageLeaderboard(
  ...
  const poolName =
    leaderboardType === "score-policies" ? (scorePoliciesPool ?? stage) : stage;

  if (leaderboardType === "team") {
    return {
      kind: "team",
      rows: await repo.getSeasonStageLeaderboard(seasonName, "team", poolName),
    };
  }
  ...
  if (leaderboardType === "score-policies") {
    return {
      kind: "score-policies",
      rows: await repo.getSeasonStageLeaderboard(
        seasonName,
        "score-policies",
        poolName,
      ),
  ...
  return {
    kind: "policy",
    rows: await repo.getSeasonStageLeaderboard(seasonName, "policy", poolName),
  };
}
