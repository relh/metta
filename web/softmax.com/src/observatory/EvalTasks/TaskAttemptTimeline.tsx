...
export const TaskAttemptTimeline: FC<{
  task: EvalTask;
  attempt: TaskAttempt;
}> = ({ task, attempt }) => {
  const assignedDiff = formatDurationBetween(
    task.created_at,
    attempt.assigned_at,
  );
  const startedDiff = formatDurationBetween(
    attempt.assigned_at,
    attempt.started_at,
  );
  const finishedDiff = formatDurationBetween(
    attempt.started_at,
    attempt.finished_at,
  );
