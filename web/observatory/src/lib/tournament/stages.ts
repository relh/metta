...
type StageFlowSelectionEntry = Pick<TournamentStageFlowEntry, 'input_pool' | 'status'>

export function stageLabel(name: string): string {
  if (name.startsWith('team-round-')) return `Team Round ${name.split('-').pop()}`
  if (name.startsWith('sample-')) return `Sample ${name.split('-').pop()}`
  if (name.startsWith('stage-')) return `Stage ${name.split('-').pop()}`
  if (name === 'policy-scores' || name.startsWith('policy-scores-')) return 'Final Scores'
  return name
}
...
export function defaultSelectedStage(
  ...
  if (!started) {
    return firstStage && stageNames.has(firstStage) ? firstStage : (stages[0]?.name ?? firstStage ?? '')
  }
  ...
  const finalStage = stageFlow.length > 0 ? stageFlow[stageFlow.length - 1].input_pool : null
  if (finalStage && stageNames.has(finalStage)) {
    return finalStage
  }

  const withMatches = stages.filter((stage) => stage.match_count > 0)
  return withMatches.length > 0 ? withMatches[withMatches.length - 1].name : (firstStage ?? stages[0]?.name ?? '')
}
...
export function stageKindForPool(
  ...
  const byOutputPool = stageFlow.find((stage) => stage.output_pool === poolName)
  return byOutputPool?.kind ?? null
}

export function nearestStageWithMatches(stages: StageStats[], selectedStage: string): string | null {
  ...
  const selectedIndex = stages.findIndex((stage) => stage.name === selectedStage)
  if (selectedIndex === -1) {
    return indexedStagesWithMatches[indexedStagesWithMatches.length - 1].stage.name
  }
