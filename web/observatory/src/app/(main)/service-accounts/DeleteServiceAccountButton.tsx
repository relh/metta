...
export const DeleteServiceAccountButton: FC<{ id: string; name: string }> = ({ id, name }) => {
  ...
  return (
    <div className="flex items-center gap-1">
      <Button onClick={handleDelete} theme="tertiary" disabled={deleting}>
        <div className="text-red-600">{deleting ? <Spinner size="sm" /> : 'Delete'}</div>
      </Button>

      <Button onClick={() => setConfirming(false)} theme="tertiary" disabled={deleting}>
        Cancel
      </Button>
    </div>
  )
