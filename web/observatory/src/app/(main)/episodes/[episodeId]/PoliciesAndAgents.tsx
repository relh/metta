...
function policyMetric(p: PolicyStatsDetail, metric: string): number | null | undefined {
...
function generateCsv(policies: PolicyStatsDetail[], labels: string[]): string {
  const allMetrics = [...new Set(policies.flatMap((p) => Object.keys(p.avg_metrics)))].sort()
...
const Section: FC<React.PropsWithChildren<{ label: string; count: string; open: boolean; onToggle: () => void }>> = ({
  label,
  count,
  open,
  onToggle,
...
export const PoliciesAndAgents: FC<{ jobId?: string }> = ({ jobId }) => {
  ...
  const [metricSort, setMetricSort] = useState<Sort>({ key: 'name', desc: false })
  const [agentSort, setAgentSort] = useState<Sort>({ key: 'agent_id', desc: false })
  ...
  useEffect(() => {
    ...
    repo
      ...
      .then((data) => {
        ...
        const ids = data.policy_stats.map((p) => p.policy_version_id).filter((id): id is string => id != null)
        if (ids.length > 0) {
          repo
  ...
  const labels = useMemo(
    () => policies.map((p) => policyLabel(p, p.policy_version_id ? policyInfo[p.policy_version_id] : undefined)),
    [policies, policyInfo]
  )
  const allMetricNames = useMemo(
    () => ['reward', ...[...new Set(policies.flatMap((p) => Object.keys(p.avg_metrics)))].sort()],
    [policies]
  )
  ...
  const sortedMetrics = useMemo(() => {
    ...
    if (metricSort.key === 'name') {
      ...
      if (pIdx < policies.length) {
        rows.sort((a, b) => numCmp(policyMetric(policies[pIdx], a), policyMetric(policies[pIdx], b)))
      }
    }
  ...
  const sortedAgents = useMemo(() => {
    const agents = policies.flatMap((p, pIdx) => p.agents.map((a) => ({ ...a, pIdx })))
    agents.sort((a, b) => {
      let cmp: number
  ...
  const onMetricSort = useCallback((key: string) => setMetricSort((s) => toggleSort(s, key)), [])
  const onAgentSort = useCallback((key: string) => setAgentSort((s) => toggleSort(s, key)), [])
      ...
      <div className="space-y-2">
        ...
        >
          <div className="overflow-x-auto">
            <Table theme="inner">
              <TableHeader>
                <SortTH sortKey="name" active={metricSort} onSort={onMetricSort}>
                  Metric
                </SortTH>
                {policies.map((p, i) => (
                  <SortablePolicyHeader
                    ...
                    policy={p}
                    info={p.policy_version_id ? policyInfo[p.policy_version_id] : undefined}
                    sortKey={`p:${i}`}
                    active={metricSort}
                    onSort={onMetricSort}
                  />
              ...
              <TableBody>
                {sortedMetrics.map((name) => (
                  <TR key={name}>
                    <TD className={name === 'reward' ? 'font-medium' : ''}>{name}</TD>
                    {policies.map((p) => (
                      <TD key={p.policy_version_id ?? p.position}>
                        <span className="font-mono">{fmt(policyMetric(p, name))}</span>
                      </TD>
                    ))}
                  </TR>
                ))}
        ...
        >
          <div className="overflow-x-auto">
            <Table theme="inner">
              <TableHeader>
                <SortTH sortKey="agent_id" active={agentSort} onSort={onAgentSort}>
                  Agent
                </SortTH>
                <SortTH sortKey="policy" active={agentSort} onSort={onAgentSort}>
                  Policy
                </SortTH>
                <SortTH sortKey="reward" active={agentSort} onSort={onAgentSort}>
                  Reward
                </SortTH>
                {agentMetricCols.map((name) => (
                  <SortTH key={name} sortKey={name} active={agentSort} onSort={onAgentSort}>
                    {name}
                  </SortTH>
                ))}
              </TableHeader>
