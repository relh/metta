...
const TimeDiff: FC<{ from: string | null; to: string | null }> = ({ from, to }) => {
  const diff = formatDurationBetween(from, to)
  return <span className={clsx(getTimeDiffColor(from, to))}>{diff ? `+${diff}` : ''}</span>
}
...
export const Timeline: FC<{ job: JobRequest }> = ({ job }) => (
  <LabelValueTable>
    ...
    <LabelRow label="Dispatched" extra={<TimeDiff from={job.created_at} to={job.dispatched_at} />}>
      {formatDate(job.dispatched_at)}
    </LabelRow>
    <LabelRow label="Running" extra={<TimeDiff from={job.dispatched_at} to={job.running_at} />}>
      {formatDate(job.running_at)}
    </LabelRow>
    <LabelRow label="Completed" extra={<TimeDiff from={job.running_at} to={job.completed_at} />}>
