...
export const LinkTabs: FC<LinkTabsProps> = ({ tabs, stageKindsByPool, defaultStage }) => {
  const pathname = usePathname()
  const searchParams = useSearchParams()
  const requestedStage = searchParams?.get('stage')
  const hasRequestedStageKind = !!(requestedStage && stageKindsByPool?.[requestedStage])
  const selectedStage = hasRequestedStageKind ? requestedStage : (defaultStage ?? requestedStage ?? null)
  const selectedStageKind = selectedStage && stageKindsByPool ? stageKindsByPool[selectedStage] : null

  const visibleTabs = tabs.filter((tab) => {
    if (!tab.allowedStageKinds || tab.allowedStageKinds.length === 0) return true
