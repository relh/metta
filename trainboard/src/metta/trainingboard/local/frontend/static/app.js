...
function readBasePath() {
  const meta = document.querySelector('meta[name="trainboard-base-path"]')
  if (!meta) return ''
...
function betItemHtml(task, impactMetrics, executionMetrics) {
  ...
  const orderedImpactMetrics = impactMetrics && impactMetrics.length ? impactMetrics : Object.keys(IMPACT_METRIC_META)
  const orderedExecutionMetrics =
    executionMetrics && executionMetrics.length ? executionMetrics : Object.keys(EXECUTION_METRIC_META)
  const metricPills = [
