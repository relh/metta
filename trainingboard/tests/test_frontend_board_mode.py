...
def test_app_enables_auto_refresh_and_buckets() -> None:
    ...
    assert 'fetch("/api/v1/board")' in app_js
    assert 'bucketHtml("Now", "now", nowTasks)' in app_js
    assert 'bucketHtml("Next", "next", nextTasks)' in app_js
    assert 'bucketHtml("Later", "later", laterTasks)' in app_js
