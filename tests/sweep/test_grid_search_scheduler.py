...
def test_grid_scheduler_ignores_non_categorical_leaves():
    ...
    params = {
        "policy_assets.learner0.optimizer.device": ["cpu", "cuda"],
        "policy_assets.learner0.optimizer.learning_rate": 1e-3,  # ignored
    }
    ...
    assert suggestion["policy_assets.learner0.optimizer.device"] in {"cpu", "cuda"}
    assert "policy_assets.learner0.optimizer.learning_rate" not in suggestion
...
def test_grid_scheduler_accepts_list_and_nested_config():
    ...
    assert s["policy_assets.learner0.optimizer.device"] in {"cpu", "cuda"}
