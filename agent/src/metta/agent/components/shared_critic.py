...
class SharedCriticConfig(ComponentConfig):
    ...
    ``agents_per_env_slice`` controls how many agents the critic groups together.
    When set explicitly on the config, that value is used (e.g. the number of
    agents in a trajectory-isolation slice).  Otherwise it falls back to the
    total ``env.num_agents``.

    Assumptions for the contiguous env-id layout (where N = agents_per_env_slice):
    ...
    def make_component(self, env=None):
        n = self.agents_per_env_slice
        agents_per_env_slice = n if n is not None else int(env.num_agents)
        return SharedCritic(config=self, agents_per_env_slice=agents_per_env_slice)


class SharedCritic(nn.Module):
    """Shared critic that concatenates per-env agent latents and broadcasts values."""

    def __init__(self, config: SharedCriticConfig, *, agents_per_env_slice: int):
        super().__init__()
        self.config = config
        if agents_per_env_slice <= 0:
            raise ValueError("agents_per_env_slice must be a positive integer")
        self.agents_per_env_slice = agents_per_env_slice
        ...
        if self.config.use_cross_attention:
            ...
            critic_in = self.config.in_features * self.agents_per_env_slice
    ...
    def _reshape_for_envs(
        x: torch.Tensor, agents_per_env_slice: int, *, bptt: int, allow_partial_envs: bool
        ...
        remainder = batch % agents_per_env_slice
        if remainder != 0 and not allow_partial_envs:
            raise ValueError("Batch size must be divisible by agents_per_env_slice for shared critic")

        pad_agents = (agents_per_env_slice - remainder) % agents_per_env_slice
        if pad_agents:
        ...
        # Calculate number of environments by dividing batch size by agents per env slice
        num_envs = batch // agents_per_env_slice
        # Reshape from [B*TT, D] to [batch, bptt, D] - grouping timesteps into sequences
        x_seq = x.view(batch, bptt, -1)
        # Reshape to [num_envs, agents_per_env_slice, bptt, D] - grouping agents by env slice
        x_env = x_seq.view(num_envs, agents_per_env_slice, bptt, -1)
    ...
    def _build_padding_mask(
        num_envs: int, agents_per_env_slice: int, bptt: int, pad_agents: int, device: torch.device
    ) -> torch.Tensor | None:
        if pad_agents <= 0:
            return None
        mask = torch.zeros((num_envs, agents_per_env_slice), dtype=torch.bool, device=device)
        mask[-1, -pad_agents:] = True
        mask = mask[:, None, :].expand(-1, bptt, -1).reshape(num_envs * bptt, agents_per_env_slice)
        return mask
    ...
    def forward(self, td: TensorDict) -> TensorDict:
        ...
        x_env, num_envs, bptt, pad_agents = self._reshape_for_envs(
            x,
            self.agents_per_env_slice,
            bptt=bptt,
            allow_partial_envs=self.config.allow_partial_envs,
        )
        if self.config.use_cross_attention:
            # [E, N, TT, D] -> [E*TT, N, D] for per-env-time attention
            x_attn = x_env.permute(0, 2, 1, 3).reshape(num_envs * bptt, self.agents_per_env_slice, -1)
            key_padding_mask = self._build_padding_mask(
                num_envs, self.agents_per_env_slice, bptt, pad_agents, x_attn.device
            )
            ...
            attn_env = attn_out.view(num_envs, bptt, self.agents_per_env_slice, -1).permute(0, 2, 1, 3).contiguous()
            attn_seq = attn_env.view(num_envs * self.agents_per_env_slice, bptt, -1)
            ...
            if bptt == 1:
                # [E, 1] -> [E, N, 1] -> [B, 1]
                values = values.view(num_envs, 1).expand(-1, self.agents_per_env_slice).reshape(-1, 1)
                h_values = h_values.view(num_envs, 1).expand(-1, self.agents_per_env_slice).reshape(-1, 1)
            else:
                # [E*TT, 1] -> [E, TT, 1] -> [E, N, TT, 1] -> [B, TT, 1] -> [B*TT, 1]
                values = values.view(num_envs, bptt, 1).unsqueeze(1)
                values = values.expand(-1, self.agents_per_env_slice, -1, -1).reshape(-1, bptt, 1)
                values = values.reshape(-1, 1)
                h_values = h_values.view(num_envs, bptt, 1).unsqueeze(1)
                h_values = h_values.expand(-1, self.agents_per_env_slice, -1, -1).reshape(-1, bptt, 1)
