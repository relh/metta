...
def test_fast_policy_initialize_sets_action_metadata():
    ...
    assert policy.action_probs.num_actions == int(policy_env_info.action_space.n)
    assert policy.action_probs.num_actions == len(policy_env_info.non_vibe_action_names)
...
def test_fast_policy_forward_produces_actions_and_values():
    ...
    td.set("agent_slot_ids", torch.zeros((1, 1), dtype=torch.long))
